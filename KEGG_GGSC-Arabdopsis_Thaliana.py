...
# -------------------------------------------------------------------------------------------------------------
    ...
    if(data[n].startswith('D      ')):    # 6 Spaces
       ...
       d_name_regex = search(r'[\s]{6}A[a-zA-Z0-9]{8}[\s]', data[n]) # RegEX to find gene name and synonym
...
def exportGraph(): # Export generate regulation list to a txt file
    ...
    str(graph.write(f='Arabidopsis Thaliana.graphml',format="graphml"))
...
def first_hundred_genes_check():
    ...
    for i in range(100):
        print(similarity(graph, 'AT4G37840 ', graph.vs[i]['name']))

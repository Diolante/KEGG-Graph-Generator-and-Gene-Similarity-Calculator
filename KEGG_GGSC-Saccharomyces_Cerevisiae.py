...
starts_with_t = r'([t][a-zA-Z][_][A-Za-z0-9]{3}[_][A-Za-z0-9]([A-Za-z0-9])?\s)'
...
compilation_of_patterns = compile('|'.join([starts_with_Y, starts_with_t, starts_with_Q, starts_with_RDN, starts_with_snR, starts_with_NME, starts_with_ICR, starts_with_LSR]))
    
data = []
placeholderA = 0
graph = igr.Graph(directed = True) # Creates a directed igraph instance

keg_filename = 'sce00001.keg'   # Name of the input file
...
# -------------------------------------------------------------------------------------------------------------
    ...
    if(data[n].startswith('D      ')):    # 6 Spaces
       ...
       ko_regex = findall(r'([K]\d{5}([K]\d{5})?)', data[n]) # RegEX to find KO identifier
       d_name_regex = search(compilation_of_patterns, data[n]) # RegEX to find gene name and synonym
       
       
       graph.add_vertex(idg = n,
                            name = d_name_regex.group(0).replace("      ", ""),
...
def exportGraph(): # Export generate regulation list to a txt file
    ...
    str(graph.write(f='Saccharomyces.graphml',format="graphml"))
...
# -------------------------------------------------------------------------------------------------------------
        ...
        depth_lca = graph.shortest_paths(0,  common_path, mode='in')[0][0]     # Calculates de depth of the lowest common ancestor
        
        
        if(len(common_path) == 4): #C
...
def first_hundred_genes_check():
    ...
    for i in range(100):
        print(similarity(graph, 'YGL253W ', graph.vs[i]['name']))

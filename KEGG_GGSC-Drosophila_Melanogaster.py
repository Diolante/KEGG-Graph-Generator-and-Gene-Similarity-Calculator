...
keg_filename = 'dme00001.keg'   # Name of the input file
...
# -------------------------------------------------------------------------------------------------------------
    ...
    if(data[n].startswith('D      ')):    # 6 Spaces
       ...
       d_name_regex = search(r'\Dmel_C[GR]\d{4,5}', data[n]) # RegEX to find gene name and synonym
...
def exportGraph(): # Export generate regulation list to a txt file
    ...
    str(graph.write(f='Drosophilia_Melanogaster.graphml',format="graphml"))     
...
def first_hundred_genes_check():
    ...
    for i in range(100):
        print(similarity(graph, 'Dmel_CG8094', graph.vs[i]['name']))
